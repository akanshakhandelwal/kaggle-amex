...
# Observation: https://www.kaggle.com/code/raddar/the-data-has-random-uniform-noise-added
# Discussion (great comments): https://www.kaggle.com/competitions/amex-default-prediction/discussion/328514
# Download: https://www.kaggle.com/datasets/raddar/amex-data-integer-dtypes-parquet-format
...
def floorify_zeros(x):
    ...
    no_proximity = (
        len([t for t in x if t < 0 and t >= -0.01]) == 0
        and len([t for t in x if t > 0.01 and t <= 0.02]) == 0
    )
    if not no_proximity:
        return x
    if not has_zeros:
...
def floorify_ones(x):
    ...
    no_proximity = (
        len([t for t in x if t < 1 and t >= 0.99]) == 0
        and len([t for t in x if t > 1.01 and t <= 1.02]) == 0
    )
    if not no_proximity:
        return x
    if not has_ones:
...
x.loc[
    x.S_13.between(0.67, 0.7) & (x.S_11.isin([15, 16, 17])), "S_13"
] = 0.6789168283158535
floor_vals = (
    0,
    0.0377176456223467,
...
x.to_parquet("parquet/test.parquet")
